...
//    long niso, i;
    ...
    if (sc_readspec(spec, parlist) != CPL_ERROR_NONE)
    {
        ...
        // count isolated emission lines
//        for (i = 0, niso = 0; i < cpl_table_get_nrow(spec); i++) {
//            if (cpl_table_get_int(spec, "mask", i, NULL) == 3) {
//                niso++;
//            }
//        }
//        printf("#isolated lines in SPEC: %li\n", niso);
...
//    cpl_table_dump(spec, 0, 50, NULL);
    ...
    printf("estimated FWHM: %lf +- %lf\n", fwhm, rms);

//    cpl_table_dump(linetab, 0, 5, NULL);
